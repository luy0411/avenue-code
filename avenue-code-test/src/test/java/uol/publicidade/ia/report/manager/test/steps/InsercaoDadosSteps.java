package uol.publicidade.ia.report.manager.test.steps;
...
import uol.publicidade.ia.report.manager.test.config.IntegrationConfiguration;
...
public class InsercaoDadosSteps {
    ...
    private List<MetricsReport> expectedData;
    private List<MetricsReport> realData;
    ...
    public void que_sejam_recebidos_os_dados_do_relatorio(String dataFile){
        try {
            ...
            Type type = new TypeToken<List<MetricsReport>>() {}.getType();
    ...
    public void o_servico_de_insercao_de_dados_for_chamado() {
        ...
        ResponseEntity<List<MetricsReport>> response = restTemplate.exchange(
                IA_REPORT_MANAGER_INSERT_URL, HttpMethod.POST, new HttpEntity<>(expectedData),
                new ParameterizedTypeReference<List<MetricsReport>>(){}
        );
